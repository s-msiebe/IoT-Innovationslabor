...
bool SensorBoardDevice::runSetupMode()
{
	...
	if(connection->handle())
	{
		...
		else if (req.method == "PUT" && req.target == "/")
		{
			...
			//UUID im EEPROM speichern
				String key = "@id";
				
				error = !( storage->writeInStorage(key, stringId) );
		
			//Response mit Status 204
				if(!error)
				{
					res.statusCode = 204;
		...
		else if (req.method == "POST" && req.target == "/")
		{
			...
			//Radio-Config speichern
				radioModul->setServerConfig(json["radio"]);
				
			//Radio Infos speichern (auch im EEPROM)
				String key = "radio";
				//String value = json.get<String>("radio");
				JsonVariant jsonVariant = json["radio"];
				String variantAsString;
				//json.printTo(test);
				jsonVariant.printTo(variantAsString);
				//String value = json["radio"].as<String>();
				error = !( storage->writeInStorage(key, variantAsString) );
			
			//Response mit Status 204 und dem Wert, der geändert wurde
				if(!error)
				{
					res.statusCode = 204;
...
bool SensorBoardDevice::runResetMode()
{
	String key = "@id";
	String radio = "radio";
	storage->removeFromStorage(radio);
